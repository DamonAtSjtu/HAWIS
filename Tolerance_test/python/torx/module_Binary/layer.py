...
from models.quan_conv import dorefa_a, dorefa_w
from .w2g import w2g

quantize_weight = dorefa_w
quantize_input = dorefa_a
adc = _adc.apply
...
class crxb_Conv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, ir_drop, device, gmax, gmin, gwire,
                ...
                enable_resistance_variance=False, resistance_variance_gamma=0. ,
                 freq=10e6, temp=300, groups=1, bias=False, crxb_size=64, quantize=8, enable_SAF=False,
                 enable_ec_SAF=False):
        ...
        weight_flatten = self.weight.view(self.out_channels, -1)  #[c_out, c_in*k*k]
        self.crxb_row, self.crxb_row_pads = self.num_pad(
            weight_flatten.shape[1], self.crxb_size)              #number of cross_bar; padding number 
        self.crxb_col, self.crxb_col_pads = self.num_pad(
            weight_flatten.shape[0], self.crxb_size)
        self.h_out = None
        self.w_out = None
        self.w_pad = (0, self.crxb_row_pads, 0, self.crxb_col_pads)   #The shape of padded part
        self.input_pad = (0, 0, 0, self.crxb_row_pads)      #The shape of padding part
        weight_padded = F.pad(weight_flatten, self.w_pad,
                              mode='constant', value=0)      #The shape of padded weight
        weight_crxb = weight_padded.view(self.crxb_col, self.crxb_size,
                                         self.crxb_row, self.crxb_size).transpose(1, 2) # The shape of padded weight on crossbar
    ...
    def forward(self, input):
        ...
        input_quan = quantize_input(input) * self.delta_v  # convert to voltage
        with torch.no_grad():
            delta_weight_scaler = torch.mean(torch.abs(self.weight))        
        weight_quan = quantize_weight(self.weight)
        
        weight_quan = weight_quan /delta_weight_scaler
        ...
        output_adc = output_crxb * delta_weight_scaler / (self.delta_v *  self.delta_g)
...
class crxb_Linear(nn.Linear):
    ...
    def __init__(self, in_features, out_features, ir_drop, device, gmax, gmin, gwire, gload, freq=10e6,
                 vdd=3.3, scaler_dw=1, temp=300, bias=False, crxb_size=64, quantize=8, enable_ec_SAF=False,
                enable_resistance_variance=False, resistance_variance_gamma=0. ,
