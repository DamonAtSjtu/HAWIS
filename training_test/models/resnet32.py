...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        #self.out_planes = [16,16,16,16,16,16,16,16,16,16,16, 32,32,32,\
        #   32,32,32,32,32,32,32,64,64,64,64,64,64,64,64,64,64]

        # HAWIS-A
        # self.out_planes = [80,80,80,80,80,80,80,56,72,56,72, 168,168,136,152,128,160,\
        #     128,168,156,168, 224,224, 220,196,168,196,164,192,224,168]

        # HAWIS-B
        self.out_planes = [80,80,80,80,80,80,80,56,72,56,72, 168,168,136,152,128,160, \
            128,168,156,168, 256,256, 224,256,296,224,196,224,168,224]
        self.in_planes = self.out_planes[0]

        self.conv1 = conv3x3_first(int(3*32), self.in_planes, stride=1)
        self.bn1 = nn.BatchNorm2d(self.in_planes)
        self.layer1 = self._make_layer(block, self.out_planes[0:11], num_blocks[0], stride=1)
        self.layer2 = self._make_layer(block, self.out_planes[10:21], num_blocks[1], stride=2)
        self.layer3 = self._make_layer(block, self.out_planes[20:], num_blocks[2], stride=2)
...
def resnet32():
    return ResNet(BasicBlock_bi, [5, 5, 5])
